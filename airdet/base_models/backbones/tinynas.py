...
class TinyNAS(nn.Module):
    ...
    def __init__(self, structure_info = None, out_indices = [0, 1, 2, 4, 5], out_channels = [None, None, 128, 256, 512],
                 with_spp = False, use_focus = False, need_conv1 = True, act="silu"):
        ...
        for idx, block_info in enumerate(structure_info):
            ...
            if the_block_class == 'ConvKXBNRELU':
                if use_focus:
                    the_block = Focus(block_info['in'], block_info['out'], block_info['k'])
                else:
                    the_block = ConvKXBNRELU(block_info['in'], block_info['out'], block_info['k'], block_info['s'],
                                             act=act)
                self.block_list.append(the_block)
                ...
                self.block_list.append(the_block)
            if need_conv1:
                if idx in self.out_indices and out_channels[self.out_indices.index(idx)] is not None:
                    self.conv1_list.append(nn.Conv2d(block_info['out'], out_channels[self.out_indices.index(idx)], 1))
                else:
...
def load_tinynas_net(backbone_cfg):
    ...
    for layer in struct_info:
        if 'nbitsA' in layer: del layer['nbitsA']
        if 'nbitsW' in layer: del layer['nbitsW']
